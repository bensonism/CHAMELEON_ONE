...
class Tax_Meta_Class {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme'])){
      if ($meta_box['use_with_theme'] === true){
        ...
        $this->SelfPath = $themeURL; // LMD HACK IN PLACE HERE AS IT WASNT WORKING WITH GET THEMPLATE DIRECTORY URI
      }
    }
    // Add Actions
  ...
  public function load_scripts_styles() {
    ...
    if (in_array($taxnow,$this->_meta_box['pages'])){
      ...
      wp_enqueue_style( 'tax-meta-clss', $plugin_path . '/inc/classes/custom_taxonomy_meta/css/Tax-meta-class.css' );
      // Enqueue Meta Box Scripts
      wp_enqueue_script( 'tax-meta-clss', $plugin_path . '/inc/classes/custom_taxonomy_meta/js/tax-meta-clss.js', array( 'jquery' ), null, true );
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  if (!isset($value['id']))
	...
	$multiple     = isset($field['multiple'])? $field['multiple'] : false;
	$multiple     = ($multiple)? "multiFile " : "";
	// echo "<span class='simplePanelImagePreview'><img width='200px' {$PreviewStyle} src='{$value['url']}'><br/></span>";
	echo "<span class='simplePanelImagePreview'><img width='200px' height='100px'  src='{$value['url']}'><br/></span>";
	echo "<input type='hidden' name='{$name}[id]' value='{$value['id']}'/>";
	echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
	if ($has_image)
