...
function reptilehaus_load_image_widget() {
	register_widget( 'REPTILEHAUS_Image_Widget' );
}
add_action( 'widgets_init', 'reptilehaus_load_image_widget' );

/**
 * REPTILEHAUS_Image_Widget class
 **/
class REPTILEHAUS_Image_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$widget_ops = array( 'classname' => 'widget_sp_image', 'description' => __( 'Advert or Image Widget with a Title, URL, and a Description', 'image-widget' ) );
		$control_ops = array( 'id_base' => 'widget_sp_image' );
		parent::__construct( 'widget_sp_image', __( 'Image/Advert Widget', 'image-widget' ), $widget_ops, $control_ops );
	...
	public function admin_setup() {
		wp_enqueue_media();
		wp_enqueue_script( 'reptilehaus-image-widget', get_template_directory_uri() . '/LMD_Framework/widget/image-advert-widget/resources/js/image-widget.js', array( 'jquery', 'media-upload', 'media-views' ), self::VERSION );
	...
	private function get_image_html( $instance, $include_link = true ) {
		...
		if ( ! empty( $instance['imageurl'] ) ) {
			...
			// $path_parts = pathinfo($instance['imageurl']);
			// $advertsImage =  $path_parts['dirname'] . "/" . $path_parts['filename'] . "-330x350." . $path_parts['extension'];
